...
canvas  size = mdo
  ...
  (undo, redo) <- translation (0,  -200) $ liftA2 (,)
    (translation (-50, 0) $ disabledButton (not . null <$> drawings) (pure "Undo") (pure (80, 30))) 
    (translation (50, 0) $ disabledButton (not . null <$> redos) (pure "Redo") (pure (80, 30))) 

      
  scribble <- pen size currentCol
  
  drawings <- current <$> foldMerge [] 
    [ (:) <$> scribble
    , (:) <$> fmapMaybe listToMaybe (tag redos redo)
    , drop 1 <$ undo
    ]
    
  redos <- current <$>  foldMerge [] 
    [ const [] <$ scribble
    , (:) <$> fmapMaybe listToMaybe (tag drawings undo)
    , drop 1 <$ redo
    ]
